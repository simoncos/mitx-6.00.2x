...
def scaleFeatures(vals): # Feature Standardization
    ...
    def findClosest(self, linkage):
        ...
        for i in range(len(clusters)-1):
            for j in clusters[(i+1):]:
                # linkage 作为函数名参数传入，此外类内函数的调用方式有两种：
                # 一种是用类的实例 - instance.function(parameters),
                # 另一种是用类名.函数名 - Class.function(parameters) 
                distance = linkage(clusters[i],j) 
                if distance < minDis:
                    minDis = distance
                    closest = (clusters[i],j)
        return closest
